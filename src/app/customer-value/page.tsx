...
import CustomerValueInputSection from '@/components/analysis/CustomerValueInputSection';
import CustomerValueResultsSection from '@/components/analysis/CustomerValueResultsSection';
import { customerValueEngine } from './lib/gemini';
...
export default function CustomerValue() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const result = await customerValueEngine.analyze(uiState.inputText.trim());
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: 'var(--background)' }}>
      ...
      <footer className="py-6 text-center text-xs magazine-subtitle" style={{ color: 'var(--text-tertiary)' }}>
        <p>
          © 2025 姜博士客户价值六件套 - AI操作平台 | 基于客户价值六件套的深度分析
        </p>
      </footer>
    </div>
  );

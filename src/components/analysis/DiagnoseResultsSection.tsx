...
interface DiagnoseResultsSectionProps {
  result: AnalysisResult;
  onNewAnalysis: () => void;
}

export default function DiagnoseResultsSection({ result, onNewAnalysis }: DiagnoseResultsSectionProps) {
  const [copied, setCopied] = useState(false);

  const copyToClipboard = async () => {
    const fullText = `🔍 姜博士三加一诊断结果
诊断时间：${new Date(result.timestamp).toLocaleString()}

${result.result}

---
诊断完成时间：${new Date(result.timestamp).toLocaleString()}
    `.trim();
  ...
  const exportToMarkdown = () => {
    const markdownContent = `# 🔍 姜博士三加一诊断结果
...
`;
    ...
    a.href = url;
    a.download = `三加一诊断_${new Date(result.timestamp).toLocaleDateString()}.md`;
...
<body>
    ...
    <div class="meta">
        <strong>诊断时间：</strong> ${new Date(result.timestamp).toLocaleString()}
    </div>
    ...
    <div class="meta">
        <em>诊断完成时间：${new Date(result.timestamp).toLocaleString()}</em>
    </div>
</body>
</html>`;
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `三加一诊断_${new Date(result.timestamp).toLocaleDateString()}.doc`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);

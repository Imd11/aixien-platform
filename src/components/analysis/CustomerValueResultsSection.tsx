...
interface CustomerValueResultsSectionProps {
  result: AnalysisResult;
  onNewAnalysis: () => void;
}

export default function CustomerValueResultsSection({ result, onNewAnalysis }: CustomerValueResultsSectionProps) {
  const [copied, setCopied] = useState(false);

  const copyToClipboard = async () => {
    const fullText = `🎯 姜博士客户价值六件套分析结果
...
分析完成时间：${new Date(result.timestamp).toLocaleString()}
  ...
  const exportToMarkdown = () => {
    const markdownContent = `# 🎯 姜博士客户价值六件套分析结果
...
`;
    ...
    a.href = url;
    a.download = `客户价值六件套分析结果_${new Date(result.timestamp).toLocaleDateString()}.md`;
...
<body>
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `客户价值六件套分析结果_${new Date(result.timestamp).toLocaleDateString()}.doc`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);

...
import { Copy, Download, RotateCcw, CheckCircle, FileText, Briefcase } from 'lucide-react';
...
分析完成时间：${new Date(result.timestamp).toLocaleString()}
  ...
  const exportToMarkdown = () => {
    const markdownContent = `# 💼 特连光电企业家问题分析报告
...
`;
    ...
    a.href = url;
    a.download = `特连光电企业家分析_${new Date(result.timestamp).toLocaleDateString()}.md`;
...
<body>
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `特连光电企业家分析_${new Date(result.timestamp).toLocaleDateString()}.doc`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
